...
def cardiac_fully_captured(source_path, device, img_size=(1,10,256,256)):
    r"""Method which checks wether the cardiac is fully captured. The first and the last slices have to be completed back. The slices in the middle must have black edges"""

    #Load the Unet
    path_m = os.path.join('../../JIP/data_dirs/persistent/segmentation/PUnet.zip')
    model = lr.load_model('PUnet', path_m, True)
    model.to(device)
    logging.info('Model for cardiac segmentation loaded!')
    ...
    #True when the cardiac is fully captured
    return True
