...
def save_results(model, noise, paths, pathr, losses_train, losses_val, accuracy_train,
    ...
    model_path = os.path.join(os.environ["OPERATOR_PERSISTENT_DIR"], noise+"-thorax")
    ...
    with open(os.path.join(pathr, 'accuracy.txt'), 'w') as f: 
        ...
        print('\n accuracy det val \n', file=f)
        print(accuracy_det_val, file=f)
        print('\n accuracy test \n', file=f)
        print(accuracy_test, file=f)
        print('\n accuracy det test \n', file=f)
        print(accuracy_det_test, file=f)
        print('\n sensivity, specificity, precision, accuracy \n', file=f)
...
def save_only_test_results(path, losses_test, accuracy_test, accuracy_det_test, noise,  y_hat_table, metrics):
    ...
    with open(os.path.join(path, 'y_hat_table.txt'), 'w') as f: 
        print(y_hat_table, file=f)
    with open(os.path.join(path, 'accuracy.txt'), 'w') as f: 
        print('\n sensivity, specificity, precision, accuracy \n', file=f)
        print(metrics, file=f)
    ...
    heat_map.set_title(str.capitalize(noise))
    figure = heat_map.get_figure()
    figure.savefig(os.path.join(path, 'y_hat_table.png'))
