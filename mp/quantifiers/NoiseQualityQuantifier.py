...
class NoiseQualityQuantifier(ImgQualityQuantifier):
    def __init__(self, artefact_given, output_features, device='cuda:0', version='0.0'):
        ...
        # Get model path
        path_m = os.path.join(os.environ["OPERATOR_PERSISTENT_DIR"], self.artefact+"-thorax", 'model_state_dict.zip')
    ...
    def get_quality(self, x, path, gpu, cuda): #New NB
        ...
        #fully_captured = hippocampus_fully_captured(source_path = file, device=device) #NEW NB
        discard, _, _ = LungFullyCaptured(path, gpu, cuda)
        fully_captured = not discard
